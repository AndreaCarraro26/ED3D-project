...
void edit_conf(Configuration& confData) {
	...
	// this function edit the Configuration struct
	...
	while (!esc) {
		...
		std::cout << "[1] Camera/Laser height (" << confData.cameraHeight << ")\n";
		std::cout << "[2] Baseline (" << confData.baseline << ")\n";
		...
		std::cout << "[6] Y position at start (" << confData.minY << ")\n";
		std::cout << "[7] Y position at the end (" << confData.maxY << ")\n";
		std::cout << "[8] Select intersectoion function (" << type << ")\n";
		...
		case 2:
			while (!input_valid) {
				...
				std::cout << "New baseline (value accepted from 500 to 800mm): " << std::flush; std::cin >> input;
				if (std::cin.fail() || input <= 500 || input > 800) {
					std::cin.clear();
					std::cin.ignore(std::cin.rdbuf()->in_avail());
					std::cout << "Input not valid! Insert a value ranging from 500 to 800mm.\n";
					input_valid = false;
				}
			}
		...
		case 5:
			while (!input_valid) {
				...
				std::cout << "Type (T) to use use use bounds, (F) otherwise. \nIf True, bounds must be setted manually. " << std::flush;
				char ch;
				std::cin >> ch;
				if (ch == 't' || ch == 'T') {
					...
					std::cout << "Input not valide! " << std::endl;
					input_valid = false;
				}
			}
		case 6:
			while (!input_valid) {
				...
				std::cout << "New start position (referred to camera): " << std::flush; std::cin >> input;
				if (std::cin.fail()) {
					std::cin.clear();
					std::cin.ignore(std::cin.rdbuf()->in_avail());
		...
		case 7:
			while (!input_valid) {
				...
				std::cout << "New final position (referred to camera): " << std::flush; std::cin >> input;
				if (std::cin.fail()) {
					std::cin.clear();
					std::cin.ignore(std::cin.rdbuf()->in_avail());
					std::cout << "Input not valid! Insert a number value!\n";
		...
		case 8:
			while (!input_valid) {
				...
				std::cout << "Type (M) to use Moller-Trumbore, (N) to use Naive-OSG based algorithm: " << std::flush;
				char ch;
				std::cin >> ch;
				if (ch == 'm' || ch == 'M') {
...
void save_config(Configuration& confData) {
	...
	// this function save the struct data in Configuration.xml 
